    ...
    def forward(ctx, coords):
        ...
        Args:
            coords: [N, 3], int32, in [0, 128)
                (for some reason there is no uint32 tensor in torch...)
...
class _march_rays_train(Function):
    ...
    @custom_fwd(cast_inputs=torch.float32)
    def forward(
        ctx, rays_o, rays_d, z_hats, bound, density_bitfield, C, H, nears, fars,
        step_counter=None, mean_count=-1, perturb=False, align=-1,
        force_all_rays=False, dt_gamma=0, max_steps=1024, is_ndc=False
    ):
        ...
        Args:
            ...
            mean_count: int32, estimated mean steps to accelerate training. (but will randomly drop
                rays if the actual point count exceeded this threshold.)
            perturb: bool
            align: int, pad output so its size is dividable by align, set to -1 to disable.
            force_all_rays: bool, ignore step_counter and mean_count, always calculate all rays.
                Useful if rendering the whole image, instead of some rays.
            dt_gamma: float, called cone_angle in instant-ngp, exponentially accelerate ray
                marching if > 0. (very significant effect, but generally lead to worse performance)
            max_steps: int, max number of sampled points along each ray, also affect min_stepsize.
        Returns:
            xyzs: float, [M, 3], all generated points' coords. (all rays concated, need to use
                `rays` to extract points belonging to each ray)
            dirs: float, [M, 3], all generated points' view dirs.
            deltas: float, [M, 2], all generated points' deltas. (first for RGB, second for Depth)
            rays: int32, [N, 3], all rays' (index, point_offset, point_count), e.g.,
                xyzs[rays[i, 1]:rays[i, 2]] --> points belonging to rays[i, 0]
        '''
        ...
        if is_ndc:
            if not z_hats.is_cuda:
                z_hats = z_hats.cuda()
            z_hats = z_hats.contiguous().view(-1)
        else:
            z_hats = torch.tensor(()).cuda()  # Dummy tensor

        N = rays_o.shape[0]  # num rays
        M = N * max_steps  # init max points number in total

        # running average based on previous epoch (mimic `measured_batch_size_before_compaction` in
        # instant-ngp); estimates the max points number to enable faster training, but will lead to
        # random ignored rays if underestimated.
        ...
        deltas = torch.zeros(M, 4, dtype=rays_o.dtype, device=rays_o.device)
        ...
        # tmp_xyzs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device)
        # tmp_rays = torch.empty(N, 3, dtype=torch.int32, device=rays_o.device)
        # tmp_counter = torch.zeros(2, dtype=torch.int32, device=rays_o.device)
        # _backend.march_rays_unbounded_train(
        #     rays_o, rays_d, density_bitfield, bound, dt_gamma, max_steps,
        #     N, C, H, M, nears, fars, tmp_xyzs, dirs, deltas, tmp_rays, tmp_counter, noises)

        _backend.march_rays_train(
            rays_o, rays_d, z_hats, density_bitfield, bound, dt_gamma, max_steps, is_ndc,
            N, C, H, M, nears, fars, xyzs, dirs, deltas, rays, step_counter, noises)

        # rays = rays[torch.sort(rays[:, 0], stable=True)[1]]
        # tmp_rays = tmp_rays[torch.sort(tmp_rays[:, 0], stable=True)[1]]
        # print(torch.sum(rays[:, 0] == tmp_rays[:, 0]), torch.sum(rays[:, 2] == tmp_rays[:, 2]))
        # print(rays[:10, 2])
        # print(tmp_rays[:10, 2])
...
class _composite_rays_train(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, deltas, rays, T_thresh=1e-4, is_ndc=False):
        ...
        _backend.composite_rays_train_forward(
            sigmas, rgbs, deltas, rays, M, N, C, T_thresh, is_ndc, weights_sum, depth, image)
    ...
    def backward(ctx, grad_weights_sum, grad_depth, grad_image):
        ...
        _backend.composite_rays_train_backward(
            grad_weights_sum, grad_image, sigmas, rgbs, deltas, rays, ctx.is_ndc,
            weights_sum, image, M, N, C, T_thresh, grad_sigmas, grad_rgbs, rgbs_buf)
...
class _march_rays(Function):
    ...
    @custom_fwd(cast_inputs=torch.float32)
    def forward(
        ctx, n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, z_hats, bound, density_bitfield,
        C, H, near, far, align=-1, perturb=False, dt_gamma=0, max_steps=1024, is_ndc=False
    ):
        ''' march rays to generate points (forward only, for inference)
        Args:
            n_alive: int, number of alive rays
            n_step: int, how many steps we march
            rays_alive: int, [N], the alive rays' IDs in N
                (N >= n_alive, but we only use first n_alive)
        ...
        Returns:
            ...
            deltas: float, [n_alive * n_step, 2], all generated points' deltas (here we record two
                deltas, the first is for RGB, the second for depth).
        '''
        ...
        _backend.march_rays(
            n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, z_hats, bound, dt_gamma,
            max_steps, is_ndc, C, H, density_bitfield, near, far, xyzs, dirs, deltas, noises)
...
class _composite_rays(Function):
    ...
    def forward(
        ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, is_ndc,
        weights_sum, depth, image, T_thresh=1e-2
    ):
        ...
        _backend.composite_rays(
            n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, rgbs, deltas, C, is_ndc,
