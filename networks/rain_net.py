...
class RainNet(nn.Module):
    ...
    def get_style_feat(self, input):
        ...
        for style_feat in style_feats:
            style_feat_mean, style_feat_std, style_feat_mean_std = \
                self.calc_feat_mean_std(style_feat)
...
class StyleVAEExtractor(nn.Module):
    def __init__(self) -> None:
        super().__init__()

        vgg_pretrained_path = 'pretrained/vgg_normalised.pth'
        fc_encoder_pretrained_path = 'pretrained/fc_encoder_iter_160000.pth'
        ...
        fc_encoder.load_state_dict(torch.load(fc_encoder_pretrained_path))
        self.net = RainNet(vgg, fc_encoder)

        for param in self.net.parameters():
            param.requires_grad = False

    def forward(self, style_images):
        return self.net.get_hyper_input(style_images)
