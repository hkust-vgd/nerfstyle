...
class _grid_encode(Function):
    ...
    def forward(
        ...
        S = np.log2(per_level_scale)  # log resolution multiplier; apply log2 at CUDA stage
        ...
        if calc_grad_inputs:
            ...
            dy_dx = torch.empty(1, device=inputs.device, dtype=embeddings.dtype)  # placeholder
...
class GridEncoder(nn.Module):
    def __init__(
        self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16,
        ...
        for i in range(num_levels):
            ...
            params_in_level = min(self.max_params, (
                resolution if align_corners else resolution + 1) ** input_dim)  # limit max number
